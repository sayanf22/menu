# AddMenu - Legal Policies & Information

**Website:** https://addmenu.in  
**Company:** AddMenu  
**Location:** Tripura, India  
**Contact:** support@addmenu.in | +91 700-583-2798

---

## Table of Contents
1. [Terms and Conditions](#terms-and-conditions)
2. [Privacy Policy](#privacy-policy)
3. [Cancellation & Refund Policy](#cancellation--refund-policy)
4. [Shipping & Delivery Policy](#shipping--delivery-policy)
5. [Contact Us](#contact-us)
6. [Pricing](#pricing)
7. [About Us](#about-us)

---

# Terms and Conditions

**Last updated:** December 28, 2025

## 1. Introduction

Welcome to AddMenu ("Company", "we", "our", "us"). These Terms and Conditions govern your use of our website at https://addmenu.in and our digital menu services.

By accessing or using the Service, you agree to be bound by these Terms.

## 2. Definitions

- **"Service"** - AddMenu website and digital menu platform
- **"User"** - Any individual or entity using our Service
- **"Subscriber"** - Users with paid subscriptions
- **"Content"** - Menu items, images, text uploaded by Users
- **"QR Code"** - Scannable codes for accessing digital menus

## 3. Service Description

AddMenu provides:
- Digital menu creation and hosting platform
- QR code generation for restaurant menus
- Menu management dashboard with drag & drop ordering
- Analytics and customer feedback tools
- Bell calling and call service features
- Customer support services

## 4. Account Registration

### 4.1 Eligibility
- Be at least 18 years of age
- Have legal capacity to enter into a binding agreement
- Provide accurate and complete registration information
- Be authorized to represent the business (if applicable)

### 4.2 Account Responsibilities
- Maintain confidentiality of account credentials
- Responsible for all activities under your account
- Notify us immediately of unauthorized access
- Keep contact information up to date

## 5. Payment Terms

### 5.1 Pricing
- All prices in Indian Rupees (INR)
- Prices inclusive of applicable taxes unless stated otherwise
- We reserve the right to change prices with prior notice

### 5.2 Payment Methods (via Razorpay)
- Credit Cards (Visa, MasterCard, American Express)
- Debit Cards
- UPI (Google Pay, PhonePe, Paytm, etc.)
- Net Banking
- Digital Wallets

### 5.3 Billing
- Subscriptions billed in advance
- Monthly subscriptions renew automatically each month
- Annual subscriptions renew automatically each year
- Yearly plans include 1 month FREE (pay for 11 months)
- Email notifications sent before renewal

## 6. User Content

### 6.1 Ownership
You retain all ownership rights to content you upload.

### 6.2 License Grant
By uploading content, you grant AddMenu a non-exclusive, worldwide, royalty-free license to use, display, and distribute your content solely for providing our Service.

### 6.3 Content Guidelines
Do not upload content that:
- Infringes on intellectual property rights
- Contains false or misleading information
- Is offensive, defamatory, or inappropriate
- Violates any applicable laws
- Contains malware or harmful code

## 7. Prohibited Uses

- Use the Service for illegal purposes
- Attempt unauthorized access to our systems
- Interfere with or disrupt the Service
- Resell or redistribute without authorization
- Use automated systems to access the Service
- Impersonate another person or entity
- Upload viruses or malicious code

## 8. Intellectual Property

The AddMenu name, logo, website design, and all related intellectual property are owned by AddMenu. You may not use our trademarks without prior written consent.

## 9. Service Availability

- We strive to maintain 99.9% uptime
- Scheduled maintenance with prior notice
- Not liable for downtime due to factors beyond our control
- We reserve the right to modify or discontinue features

## 10. Termination

### By User
You may terminate your account at any time through dashboard settings or by contacting us.

### By AddMenu
We may suspend or terminate your account if you:
- Violate these Terms and Conditions
- Fail to pay subscription fees
- Engage in fraudulent activity
- Abuse our Service or support team

## 11. Limitation of Liability

To the maximum extent permitted by law, AddMenu shall not be liable for:
- Any indirect, incidental, or consequential damages
- Loss of profits, data, or business opportunities
- Damages arising from service interruptions
- Third-party actions or content

Our total liability shall not exceed the amount paid by you in the 12 months preceding the claim.

## 12. Dispute Resolution

- First attempted through good-faith negotiation
- Subject to exclusive jurisdiction of courts in Tripura, India
- Governed by the laws of India

---

# Privacy Policy

**Last updated:** December 28, 2025

At AddMenu, we are committed to protecting your privacy and ensuring the security of your personal information.

## 1. Information We Collect

### 1.1 Personal Information
- **Account Information:** Name, email address, phone number, password
- **Business Information:** Restaurant name, address, business type
- **Payment Information:** Processed securely through Razorpay (we do not store card details)
- **Communication Data:** Messages, feedback, and support requests

### 1.2 Menu Content
- Menu item names and descriptions
- Prices and categories
- Food images
- Restaurant logos and branding

### 1.3 Automatically Collected Information
- **Device Information:** Browser type, operating system, device type
- **Usage Data:** Pages visited, time spent, features used
- **Log Data:** IP address, access times, referring URLs
- **Analytics Data:** Menu views, QR code scans, customer interactions

### 1.4 Cookies and Tracking
We use cookies to:
- Keep you logged in
- Remember your preferences
- Analyze usage patterns
- Improve our Service

## 2. How We Use Your Information

- **Provide Services:** Create and host your digital menu, generate QR codes
- **Process Payments:** Handle subscription payments through Razorpay
- **Communicate:** Send service updates, respond to inquiries, provide support
- **Improve:** Analyze usage to enhance features and user experience
- **Secure:** Detect and prevent fraud, abuse, and security threats
- **Legal Compliance:** Comply with applicable laws and regulations

## 3. Information Sharing

We do NOT sell, trade, or rent your personal information.

### 3.1 Service Providers
- **Razorpay:** Payment processing
- **Supabase:** Database and authentication services
- **Cloudflare:** Content delivery, security, and image storage (R2)
- **Analytics providers:** Usage analysis (anonymized data)

### 3.2 Legal Reasons
- Comply with legal obligations
- Respond to lawful requests from authorities
- Protect our rights and safety
- Prevent fraud or illegal activities

## 4. Data Security

- **Encryption:** All data encrypted in transit (SSL/TLS) and at rest
- **Secure Storage:** Data stored on secure, access-controlled servers
- **Password Security:** Bcrypt hashing for all passwords
- **Access Controls:** Limited employee access on need-to-know basis
- **Regular Audits:** Periodic security assessments and updates
- **Payment Security:** PCI-DSS compliant payment processing via Razorpay

## 5. Data Retention

- **Active Accounts:** As long as your account is active
- **After Cancellation:** 30 days for potential reactivation
- **Legal Requirements:** As required by applicable laws (typically 7 years for financial records)
- **Anonymized Data:** May be retained indefinitely for analytics

## 6. Your Rights

- **Access:** Request a copy of your personal data
- **Correction:** Update or correct inaccurate information
- **Deletion:** Request deletion of your data (subject to legal requirements)
- **Portability:** Export your data in a machine-readable format
- **Opt-out:** Unsubscribe from marketing communications
- **Withdraw Consent:** Withdraw previously given consent

To exercise these rights, contact us at support@addmenu.in

## 7. Children's Privacy

Our Service is not intended for children under 18 years of age. We do not knowingly collect personal information from children.

## 8. Grievance Officer

**Name:** AddMenu Support Team  
**Email:** support@addmenu.in  
**Phone:** +91 700-583-2798  
**Address:** Tripura, India  
**Response Time:** Within 24-48 hours

---

# Cancellation & Refund Policy

**Last updated:** December 28, 2025

## ✓ 7-Day Money-Back Guarantee

We offer a full refund within 7 days of purchase if you're not satisfied with our service.

## 1. Cancellation Policy

### 1.1 How to Cancel
- Log into your dashboard and navigate to Account Settings
- Send an email to support@addmenu.in
- Call us at +91 700-583-2798
- Message us on WhatsApp at +91 700-583-2798

### 1.2 Cancellation Timeline
- Requests processed within 24-48 hours
- Confirmation email sent once complete
- Access continues until end of current billing period

### 1.3 Effect of Cancellation
- Account remains active until end of paid period
- QR codes stop working after subscription ends
- Data retained for 30 days for potential reactivation
- After 30 days, all data permanently deleted

## 2. Refund Policy

### 2.1 Refund Eligibility
You are eligible for a full refund if:
- Request within 7 days of initial payment
- Service does not work as described
- Technical issues our team cannot resolve
- Not satisfied with service quality

### 2.2 Non-Refundable Situations
- Requests made after 7 days from payment date
- Custom development work that has been completed
- Setup fees after account activation and use
- Renewal payments (cancel before renewal date)
- Accounts terminated due to Terms violation
- Partial month usage after 7-day period

### 2.3 Refund Process
1. **Submit Request:** Email support@addmenu.in with subject "Refund Request - [Your Email]"
2. **Provide Details:** Include registered email, payment date, and reason
3. **Review Period:** 2-3 business days
4. **Confirmation:** Email confirming approval or denial
5. **Processing:** Approved refunds processed within 5-7 business days

### 2.4 Refund Method
Refunds credited to original payment method:
- UPI payments: Same UPI ID
- Card payments: Same card (5-7 business days)
- Net Banking: Same bank account (5-10 business days)
- Wallet payments: Same wallet

## 3. Subscription Plans

### Monthly Subscriptions
- Billed monthly on same date each month
- Cancel anytime before next billing date
- No refund for partial months after 7-day period

### Annual Subscriptions
- Billed once per year
- Includes 1 month FREE (pay for 11 months, get 12)
- 7-day refund policy applies from payment date
- Pro-rated refunds considered case-by-case

## 4. Payment Disputes

If you believe you were charged incorrectly:
- Contact us immediately at support@addmenu.in
- Provide transaction details and screenshots
- We will investigate within 5 business days
- Duplicate charges refunded immediately upon verification

---

# Shipping & Delivery Policy

**Last updated:** December 28, 2025

## Digital Service - No Physical Shipping

AddMenu is a 100% digital service. All products and services are delivered electronically. No physical goods are shipped.

## 1. Nature of Service

AddMenu provides:
- Digital menu creation and hosting platform
- QR code generation for restaurant menus
- Online dashboard access for menu management
- Analytics and reporting tools
- Customer feedback collection system
- Bell calling and call service features

## 2. Digital Delivery Timeline

| Service | Delivery Time |
|---------|---------------|
| Account Access | Instant (within seconds) |
| Dashboard Access | Instant (within seconds) |
| QR Code Generation | Instant (within seconds) |
| Menu Setup Assistance | Within 24 hours |
| Custom Support | Within 24-48 hours |

## 3. Access Credentials

After successful payment:
- Login credentials sent to registered email
- Immediate dashboard access at addmenu.in
- QR codes downloadable instantly from dashboard
- All features available immediately based on subscription plan

## 4. Service Activation

Subscription activated automatically upon:
1. Successful payment confirmation from Razorpay
2. Account verification (if required)
3. Email confirmation sent to registered email

## 5. Technical Requirements

To access AddMenu services:
- Device with internet connection (computer, tablet, or smartphone)
- Modern web browser (Chrome, Firefox, Safari, Edge)
- Valid email address for account communications

## 6. Delivery Issues

If you face issues accessing your service after payment:
- Check email spam/junk folder for login credentials
- Ensure you're using the correct email address
- Clear browser cache and try again
- Contact our support team immediately

## 7. No Physical Shipping

AddMenu does not:
- Ship any physical products
- Deliver printed QR codes or materials
- Send any physical merchandise

All QR codes and materials can be downloaded digitally and printed locally by the customer if needed.

## 8. Service Availability

AddMenu services are available:
- 24 hours a day, 7 days a week
- Across all regions in India
- On any device with internet access

---

# Contact Us

## Get in Touch

**Email:** support@addmenu.in  
**Phone:** +91 700-583-2798  
**WhatsApp:** +91 700-583-2798  
**Website:** https://addmenu.in  
**Location:** Tripura, India

## Business Hours

Monday - Sunday: 9:00 AM - 9:00 PM IST  
WhatsApp monitored 24/7

## Response Time

- Email: Within 24 hours
- WhatsApp: Within a few hours
- Phone: Immediate during business hours

## Quick Support Tips

- For billing: Include your registered email
- For technical issues: Describe the problem in detail
- For refunds: Mention your payment date

## Company Information

**Registered Name:** AddMenu  
**Website:** https://addmenu.in  
**Support Email:** support@addmenu.in  
**Contact Number:** +91 700-583-2798  
**Address:** Tripura, India

## Service Areas

Serving restaurants across India including:
- Agartala, Khowai, Belonia, Udaipur, Dharmanagar
- And 30+ cities across Tripura and India

---

# Pricing

## Subscription Plans

### Basic Plan
**₹249/month** or **₹2,739/year** (1 month FREE)

Features:
- Digital Menu with QR Code
- 5 Menu Image Uploads
- Drag & Drop Image Ordering
- Basic Analytics Dashboard
- Customer Feedback Collection
- Social Media Links
- Unlimited Menu Updates
- Email Support

### Standard Plan ⭐ Popular
**₹399/month** or **₹4,389/year** (1 month FREE)

Features:
- Everything in Basic
- 15 Menu Image Uploads
- **Bell Calling Feature**
- **Call Service Feature**
- Priority Customer Support
- Advanced Analytics
- Real-time Notifications

### Advanced Plan
**₹599/month** or **₹6,589/year** (1 month FREE)

Features:
- Menu categories
- 100 menu items
- Toggle item availability
- Advanced Bell features
- Dark/Light mode toggle
- *Available at [addmenu.site](https://addmenu.site)*

### Premium Plan 🚀 Best Value
**₹999/month** or **₹10,989/year** (1 month FREE)

Features:
- Everything in Advanced
- Unlimited menu items
- Order management system
- Order notifications
- Priority support
- *Available at [addmenu.site](https://addmenu.site)*

## Feature Comparison

| Feature | Basic | Standard | Advanced | Premium |
|---------|-------|----------|----------|---------|
| Digital QR Menu | ✓ | ✓ | ✓ | ✓ |
| Menu Image Uploads | 5 | 15 | 100 | Unlimited |
| Drag & Drop Ordering | ✓ | ✓ | ✓ | ✓ |
| Basic Analytics | ✓ | ✓ | ✓ | ✓ |
| Customer Feedback | ✓ | ✓ | ✓ | ✓ |
| Social Media Links | ✓ | ✓ | ✓ | ✓ |
| Bell Calling Feature | — | ✓ | ✓ | ✓ |
| Call Service | — | ✓ | ✓ | ✓ |
| Advanced Analytics | — | ✓ | ✓ | ✓ |
| Menu Categories | — | — | ✓ | ✓ |
| Toggle Availability | — | — | ✓ | ✓ |
| Order Management | — | — | — | ✓ |
| Priority Support | — | ✓ | ✓ | ✓ |

## Why Choose AddMenu?

- **Quick Setup:** Under 5 minutes
- **Secure:** Razorpay powered payments
- **Support:** Local assistance available
- **Guarantee:** 7-day money-back guarantee
- **Yearly Savings:** Get 1 month FREE on yearly plans

---

# About Us

## What is AddMenu?

AddMenu is a digital menu solution for restaurants, cafes, and food businesses in India. We help you create beautiful QR code menus that your customers can scan and view on their phones.

## Our Mission

To help restaurants modernize their menu experience with affordable, easy-to-use digital solutions.

## What We Offer

- **Digital Menu Creation:** Upload your menu images and create a beautiful digital menu
- **QR Code Generation:** Get a unique QR code for your restaurant
- **Drag & Drop Ordering:** Organize your menu images in any order
- **Menu Management:** Update your menu anytime from your dashboard
- **Analytics:** Track menu views and customer engagement
- **Feedback Collection:** Collect customer feedback directly
- **Bell Calling (Standard+):** Let customers call for service from their table
- **Call Service (Standard+):** Direct phone call button for customers

## Why Restaurants Choose Us

1. **Affordable:** Plans starting at just ₹249/month
2. **Easy to Use:** No technical knowledge required
3. **Quick Setup:** Get started in under 5 minutes
4. **Local Support:** WhatsApp and phone support in your language
5. **Secure:** Razorpay powered secure payments
6. **Reliable:** 99.9% uptime guarantee
7. **Flexible:** Drag & drop to organize your menu

## Contact

**Website:** https://addmenu.in  
**Email:** support@addmenu.in  
**Phone/WhatsApp:** +91 700-583-2798  
**Location:** Tripura, India

---

*Last updated: December 28, 2025*
